...
export const WORDLIST = [
  ...
  "decor", "decrease", "deer", "defense", "define", "defy", "degree", "delay",
  ...
  "dragon", "dramatic", "drape", "draw", "dream", "dress", "drift", "drill",
  ...
  "genre", "gentle", "genuine", "gesture", "ghost", "giant", "gift", "giddy",
  ...
  "owner", "oxygen", "oyster", "ozone", "pack", "packet", "paddle", "page",
  ...
  "poem", "poet", "point", "polar", "police", "policy", "poll", "ponder",
  ...
  "remove", "render", "renew", "rent", "reopen", "repair", "replace", "report",
  ...
  "shrimp", "shrug", "shuffle", "shy", "sibling", "sick", "side", "sieve",
  ...
  "tennis", "tent", "term", "test", "text", "thank", "that", "them",
...
export function generateMnemonic(strength = 256) {
  ...
  const checksumBitsCount = strength / 32;
  const checksumBits = Array.from({ length: checksumBitsCount }, (_, i) => (hash[0] >> (7 - i)) & 1);
  const allBits = [...entropyBits, ...checksumBits];
  
  const words = [];
  for (let i = 0; i < allBits.length; i += 11) {
    const idxBits = allBits.slice(i, i + 11);
    const idx = idxBits.reduce((acc, bit) => (acc << 1) | bit, 0);
...
export function seedFromMnemonic(rawMnemonic, passphrase = '') {
  return mnemonicToSeedSync(rawMnemonic, passphrase);
}
