...
export function validateAddress(addr) {
  if (typeof addr !== 'string') {
    throw new InvalidAddressError(String(addr), 'Address must be a string');
  }
  const normalized = addr.toLowerCase().trim();
  try {
    const { hrp, version, program } = decodeBech32m(normalized);
    if (hrp !== 'sikka') throw new InvalidAddressError(normalized, "HRP must be 'sikka'");
    if (version !== 1) throw new InvalidAddressError(normalized, "Address version must be 1");
    if (program.length !== 32) throw new InvalidAddressError(normalized, "Address program must be 32 bytes");
    return normalized;
  } catch (err) {
    if (err instanceof InvalidAddressError) throw err;
    throw new InvalidAddressError(normalized, err.message);
  }
}
